...
bool Add::eventFilter(QObject *watched, QEvent *event)
{
...
//Add ok.
void Add::on_pushButtonOk_clicked()
{
    ...
    emit signalAdd(ui->lineEditKey->text(), ui->lineEditValue->text(), true);
    message(tr("Add: ") + ui->lineEditKey->text() + " - " + ui->lineEditValue->text(), false);
    accept();
}

//Add cancel.
void Add::on_pushButtonCancel_clicked()
{
    reject();
}
...
//Add message.
void Add::message(const QString &message, bool info)
{
    if(info)
    {
